...
export type V2Scan={id:string;mission_id?:string|null;name:string;scan_type:string;tool_name?:string|null;status:string;progress_percent:number;current_step?:string|null;rq_job_id?:string|null;started_at?:string|null;finished_at?:string|null;stopped_at?:string|null;deleted_at?:string|null;created_at:string;updated_at:string;renamed_at?:string|null;events?:V2ScanEvent[];artifacts?:V2ScanArtifact[]};
export type V2ScanEvent={id:string;scan_id:string;event_type:string;type?:string;message:string;payload_json?:Record<string,unknown>|null;payload?:Record<string,unknown>;status?:string|null;progress_percent?:number|null;current_step?:string|null;created_at:string};
...
async function request<T>(path:string, options?:RequestInit):Promise<T>{
  ...
  if(!response.ok){let details:unknown=null;try{details=await response.json()}catch{details=await response.text()}throw new ApiError(`V2 scans request failed: ${response.status}`,response.status,details)}
