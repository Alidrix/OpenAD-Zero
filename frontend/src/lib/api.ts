...
export const previewToolCommand=(payload:ToolActionPayload)=>req<{command?:string[];masked_command:string[];command_preview:string;preview_command_hash:string;decision:any}>('/api/tool-automation/preview',{method:'POST',body:JSON.stringify(payload)});
...
export const listToolRuns=(toolId?:string)=>req<ToolRunRecord[]>(`/api/tool-automation/runs${toolId?`?tool_id=${encodeURIComponent(toolId)}`:''}`);
