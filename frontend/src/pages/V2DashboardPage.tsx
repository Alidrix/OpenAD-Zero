    ...
    <div className="v2-shell space-y-6">
      <header className="v2-card p-6">
        <div className="v2-header">
          ...
          <div className="flex flex-wrap gap-2"><Link className="v2-button v2-button-primary" to="/scans">Open Scan Library</Link><Link className="v2-button v2-button-secondary" to="/v2-parsed-data">View Parsed Data</Link><Link className="v2-button v2-button-secondary" to="/v2-recommendations">View Recommendations</Link><button className="v2-button v2-button-secondary" onClick={() => refresh()}>Refresh dashboard</button></div>
        </div>
      </header>
      ...
      {summary && <><section className="grid gap-6 xl:grid-cols-[1fr_1fr_1fr]"><div className="v2-card p-6"><h2 className="v2-section-title">Parsed Intelligence</h2><div className="mt-5 grid gap-3 sm:grid-cols-2"><MiniMetric label="Parsed assets" value={summary.parsed.assets} /><MiniMetric label="Parsed services" value={summary.parsed.services} /><MiniMetric label="Parsed findings" value={summary.parsed.findings} /><MiniMetric label="Parsed signals" value={summary.parsed.signals} /><MiniMetric label="Parse diagnostics" value={summary.parsed.diagnostics} /></div></div><div className="v2-card p-6"><h2 className="v2-section-title">AD Surface</h2><div className="mt-5 grid gap-3 sm:grid-cols-2"><MiniMetric label="SMB hosts" value={summary.ad_surface.smb_hosts} /><MiniMetric label="LDAP hosts" value={summary.ad_surface.ldap_hosts} /><MiniMetric label="Kerberos hosts" value={summary.ad_surface.kerberos_hosts} /><MiniMetric label="WinRM hosts" value={summary.ad_surface.winrm_hosts} /><MiniMetric label="RDP hosts" value={summary.ad_surface.rdp_hosts} /><MiniMetric label="DC hints" value={summary.ad_surface.domain_controller_hints} /></div></div><div className="v2-card p-6"><h2 className="v2-section-title">Asset OS summary</h2><div className="mt-5 grid gap-3"><MiniMetric label="Windows hosts" value={summary.assets.windows_hosts} /><MiniMetric label="Linux hosts" value={summary.assets.linux_hosts} /><MiniMetric label="Unknown hosts" value={summary.assets.unknown_hosts} /></div></div></section>
      ...
      <section className="v2-safety-banner"><p>PostgreSQL is the source of truth</p><p>No execution</p><p>No parsing trigger from dashboard</p><p>No RQ job from dashboard</p><p>No subprocess</p></section>
    </div>
  );
}
