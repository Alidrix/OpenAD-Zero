...
def extract_signals(scan: Scan, events: list[ScanEvent], artifacts: list[ScanArtifact]) -> set[str]:
    signals: set[str] = set()
    ...
    for event in events:
        ...
        for key, value in payload.items():
            if key == 'signals' and isinstance(value, list):
                signals.update(str(item) for item in value if str(item) in ALLOWED_SIGNALS)
...
def build_recommendations(
    ...
    if db is not None:
        parsed_signals = {
            row.signal
            for row in db.query(ParsedSignal).filter_by(scan_id=scan.id).all()
            if row.signal in ALLOWED_SIGNALS
        }
