...
class ParseContext:
    ...
    def asset(
        ...
        if a is None:
            ...
            a.hostname = a.hostname or hostname
            a.fqdn = a.fqdn or fqdn
            a.mac_address = a.mac_address or mac
            a.os_family = a.os_family or os_family
            a.os_name = a.os_name or os_name
        return a
    ...
    def service(
        ...
        a = self.asset(source_type, source_id, ip)
    ...
    def diagnostic(
        self, source_type: str, source_id: str | None, level: str, message: str, details: dict | None = None
    ):
        self.db.add(
            ParseDiagnostic(
                scan_id=self.scan_id,
...
def parse_persisted_scan(db: Session, scan_id: str) -> ParsePersistedResult:
    ...
    db.commit()
    return ParsePersistedResult(
        scan_id=scan_id,
        assets_created=db.query(ParsedAsset).filter_by(scan_id=scan_id).count(),
        services_created=db.query(ParsedService).filter_by(scan_id=scan_id).count(),
        findings_created=db.query(ParsedFinding).filter_by(scan_id=scan_id).count(),
        signals_created=db.query(ParsedSignal).filter_by(scan_id=scan_id).count(),
        diagnostics_created=db.query(ParseDiagnostic).filter_by(scan_id=scan_id).count(),
        status='parsed',
    )
