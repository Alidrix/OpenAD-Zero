...
DATA_DIR = Path(os.getenv("OPENAD_TOOL_RUN_DIR", "data/tool-runs"))
FINDINGS_DIR = Path(os.getenv("OPENAD_FINDINGS_DIR", "data/findings"))
...
def persist_run(record: dict) -> None:
    ...
    (DATA_DIR / f"{record['run_id']}.json").write_text(json.dumps(record, indent=2, default=_json_default), encoding="utf-8")
    for finding in record.get("findings", []):
        (FINDINGS_DIR / f"{finding['id']}.json").write_text(json.dumps(finding, indent=2, default=_json_default), encoding="utf-8")
...
def load_findings() -> list[dict]:
    if not FINDINGS_DIR.exists():
        return []
    return [json.loads(p.read_text(encoding="utf-8")) for p in sorted(FINDINGS_DIR.glob("*.json"))]


def execute_tool_request(request: ToolExecutionRequest, argv: list[str], command_preview: str, timeout_seconds: int = 300) -> ToolExecutionResult:
    ...
    try:
        ...
        completed = subprocess.run(argv, shell=False, capture_output=True, text=True, timeout=timeout_seconds, cwd=os.getcwd(), env=safe_env)
    ...
    result = ToolExecutionResult(run_id, request.tool_id, request.template_id, request.target, status, returncode, stdout_lines, stderr_lines, findings, [], started_at, finished_at)
    record = {**asdict(result), "command_preview": command_preview, "masked_command": mask_command(argv, request.params), "preview_command_hash": actual_hash}
    persist_run(record)
    return result
