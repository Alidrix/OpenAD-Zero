...
_SENSITIVE_NAME_RE = re.compile(r"(?:password|pass|pwd|hash|ntlm|token|secret|key|api[_-]?key|authorization|cookie|session|SMBPass|SMBUser)", re.I)
_ASSIGNMENT_RE = re.compile(r"(?P<name>\b(?:password|pass|pwd|hash|ntlm(?:_hash)?|token|secret|key|api[_-]?key|authorization|cookie|session|SMBPass|SMBUser)\b)(?P<sep>\s*[:=]\s*)(?P<value>[^\s,;]+)", re.I)
_SET_RE = re.compile(r"(?P<prefix>\bset\s+(?:password|pass|pwd|hash|ntlm(?:_hash)?|token|secret|key|api[_-]?key|authorization|cookie|session|SMBPass|SMBUser)\s+)(?P<value>[^\s,;]+)", re.I)
...
_COMMAND_SECRET_FLAGS = {"-p", "--password", "-H", "--hash", "--hashes", "--ntlm", "--token", "--secret", "--key", "--api-key", "apikey", "SMBPass", "SMBUser"}
