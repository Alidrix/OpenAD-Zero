...
IntegrationStatus = Literal[
    'safe_auto', 'assisted_safe', 'executable_after_human_approval', 'manual_only', 'blocked_auto', 'planned'
]
...
VALID_INTEGRATION_STATUSES = {
    'safe_auto',
    'assisted_safe',
    'executable_after_human_approval',
    'manual_only',
    'blocked_auto',
    'planned',
}
...
def _template_decision(tool: dict, selected_template_id: str | None) -> ToolPolicyDecision | None:
    ...
    if not templates:
        return ToolPolicyDecision(
            False,
            'No declared runnable template exists for this tool.',
            tool.get('risk_level', 'low'),
            bool(tool.get('requires_human_approval')),
            bool(tool.get('requires_terms_acceptance')),
        )
    ...
    if selected_template_id not in COMMAND_TEMPLATES or selected_template_id not in templates:
        return ToolPolicyDecision(
            False,
            'Selected template is not allowed for this tool.',
            tool.get('risk_level', 'low'),
            bool(tool.get('requires_human_approval')),
            bool(tool.get('requires_terms_acceptance')),
        )
...
def evaluate_tool_action(
    *,
    ...
    def deny(reason):
        return ToolPolicyDecision(False, reason, risk, needs_approval, needs_terms)

    def allow(reason):
        return ToolPolicyDecision(True, reason, risk, needs_approval, needs_terms)
    ...
    template_denial = _template_decision(
        tool, selected_template_id or (tool_id if declared_template_ids and tool_id in declared_template_ids else None)
    )
    if template_denial is not None:
        return template_denial
    if status in {'safe_auto', 'assisted_safe'} and (
        contains_authorised_keyword(template) or contains_authorised_keyword(argv)
    ):
    ...
    if status == 'executable_after_human_approval':
        ...
        return ToolPolicyDecision(
            True,
            'Advanced workflow is allowed after preview, matching command hash, human approval, terms acceptance and scope validation.',
            'high',
            True,
            True,
        )
