...
def suggest_metasploit_searches(findings: list[ParsedFinding]) -> list[ExploitSearchSuggestion]:
    ...
    for f in findings:
        ...
        for cve in fields.get("cves", []) if isinstance(fields.get("cves"), list) else ([fields.get("cve")] if fields.get("cve") else []):
            add(f, fields.get("service") if isinstance(fields.get("service"), str) else None, fields.get("port") if isinstance(fields.get("port"), int) else None, str(cve), str(cve), "metasploit_search_by_cve", "Parsed finding contains a CVE.")
        ...
        if service in {"microsoft-ds", "netbios-ssn", "smb"} or port in {139,445}:
            ...
            add(f, "smb", port, None, "smb_version", "metasploit_smb_version", "SMB version scanner can refine exploit research.")
