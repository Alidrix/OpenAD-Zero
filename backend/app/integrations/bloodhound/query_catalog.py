...
class QueryCatalogError(BloodHoundError):
    pass
...
class QueryCatalog:
    ...
    def load(self):
        ...
        for p in sorted(self.pack_dir.glob('*.yml')):
            ...
            for q in data or []:
                if not q.get('read_only') or WRITE_RE.search(q.get('cypher', '')):
                    raise QueryCatalogError(f'Unsafe query {q.get("id")}')
                self.queries[q['id']] = q

    def list_public(self):
        return [
            {k: q.get(k) for k in ['id', 'name', 'description', 'risk_level', 'read_only', 'parameters']}
            for q in self.queries.values()
        ]

    def get(self, qid):
        if qid not in self.queries:
            raise QueryCatalogError(f'Unknown query_id {qid}')
    ...
    def render(self, qid, params: dict):
        ...
        for spec in q.get('parameters', []):
            ...
            if required and n not in params:
                raise QueryCatalogError(f'Missing parameter {n}')
            if n not in params:
                continue
            v = params[n]
            if spec.get('type') == 'integer':
                ...
                if allowed:
                    v = [x for x in v if x in allowed]
            else:
                v = str(v)
            clean[n] = v
        cypher = q['cypher']
        for k, v in clean.items():
            cypher = cypher.replace('{{' + k + '}}', str(v if not isinstance(v, list) else v))
