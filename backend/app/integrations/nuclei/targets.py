...
def ensure_web_targets_for_mission(db: Session, mission_id: str) -> list[WebTarget]:
    ...
    for host,svc in rows:
        if not is_web_service(svc):
            continue
        scheme=scheme_for(svc); url=url_for(host.ip, svc.port, scheme)
        if url not in existing:
            w=WebTarget(mission_id=mission_id,host_id=host.id,url=url,ip=host.ip,port=svc.port,scheme=scheme,source='nmap')
            db.add(w); db.flush(); existing[url]=w; created.append(w)
    db.commit()
