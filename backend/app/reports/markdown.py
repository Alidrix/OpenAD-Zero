...
def _recommendations(data):
    ...
    for f in data.get('findings',[]):
        ...
        if 'smb signing' in blob: rec.add('Enable SMB signing where appropriate.')
        if 'rdp' in blob or f.get('port')==3389: rec.add('Restrict RDP access and enforce MFA/jump host.')
        if 'winrm' in blob or f.get('port') in (5985,5986): rec.add('Restrict WinRM and monitor usage.')
        if f.get('source')=='nuclei' and sev in ('high','critical'): rec.add('Validate and remediate exposed web issue.')
        if f.get('source')=='bloodhound' or 'path' in blob: rec.add('Review permissions, group memberships and delegation paths.')
...
def render_markdown_report(data: dict) -> str:
    ...
    for h in data.get('hosts',[]):
        ...
        out.append(f"| {_v(h.get('ip'))} | {_v(h.get('hostname'))} | {_v(h.get('os_guess'))} | {_yn(h.get('is_domain_controller_candidate'))} | {_v(sv)} |")
    out += ['', '## 6. SMB / Windows Enumeration']
    if data.get('smb_facts'):
        for sf in data['smb_facts']: out.append(f"- {sf.get('ip')}: signing required={_yn(sf.get('smb_signing_required'))}, SMBv1={_yn(sf.get('smbv1_enabled'))}, null session={_yn(sf.get('null_session_possible'))}")
    else: out.append('- No SMB facts available.')
    if data.get('smb_shares'):
        out.append('- Anonymous shares:'); [out.append(f"  - {sh.get('ip')} {sh.get('name')} ({sh.get('access')})") for sh in data['smb_shares'] if sh.get('anonymous')]
    out += ['', '## 7. Web Exposure Findings']
    for w in data.get('web_targets',[]): out.append(f"- Web target: {w.get('url')}")
    for f in data.get('findings_by_source',{}).get('nuclei',[]): out.append(f"- {f.get('severity')}: {f.get('template_id')} matched {f.get('matched_at')} on {f.get('host') or f.get('ip')}")
    out += ['', '## 8. Active Directory / BloodHound Analysis']
    for c in data.get('bloodhound_collections',[]): out.append(f"- {c.get('filename')} SHA256={c.get('sha256')} zip_valid={_yn(c.get('zip_valid'))} ingestion={c.get('ingestion_status')}")
    for s in data.get('bloodhound_stats',[]): out.append(f"- Stats {s.get('domain_name')}: users={s.get('users_count')} computers={s.get('computers_count')} groups={s.get('groups_count')}")
    for f in data.get('findings_by_source',{}).get('bloodhound',[]): out.append(f"- {f.get('severity')}: {f.get('title')}")
    out += ['', '## 9. Findings','| Severity | Source | Title | Host/IP | Confidence |','|---|---|---|---|---|']
    for f in data.get('findings',[]): out.append(f"| {_v(f.get('severity'))} | {_v(f.get('source'))} | {_v(f.get('title'))} | {_v(f.get('host') or f.get('ip'))} | {_v(f.get('confidence'))} |")
    out += ['', '## 10. Evidence','| Label | Category | Filename | SHA256 | Source |','|---|---|---|---|---|']
    for e in data.get('evidence',[]): out.append(f"| {_v(e.get('label'))} | {_v(e.get('category'))} | {_v(e.get('filename'))} | {_v(e.get('sha256'))} | {_v(e.get('source'))} |")
    out += ['', '## 11. Recommended Next Steps'] + [f'- {r}' for r in _recommendations(data)]
    out += ['', '## 12. Technical Appendix','### Jobs']
    for j in data.get('jobs',[]): out.append(f"- {j.get('id')} {j.get('tool')}/{j.get('type')} status={j.get('status')} preview=`{j.get('command_preview')}`")
    out += ['### Evidence paths'] + [f"- {e.get('stored_path')}" for e in data.get('evidence',[])] + ['### Report metadata', f"- Mission ID: {m.get('id')}"]
    return '\n'.join(out)+'\n'
