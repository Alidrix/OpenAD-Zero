...
def generate_report(db: Session, mission_id: str, include_sections: list[str] | None = None) -> Report:
    ...
    data['progress'] = calculate_progress_score(db, mission_id)
    sections = include_sections or DEFAULT_SECTIONS
    report = Report(
        mission_id=mission_id,
        status='generating',
        title=f'OpenAD Zero Report - {data["mission"]["name"]}',
    ...
    meta_path = base / 'report_metadata.json'
    ...
    report.sections_json = {'sections': sections, 'counts': metadata['counts']}
    db.add(report)
    db.commit()
    db.refresh(report)
    return report
