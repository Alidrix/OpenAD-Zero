...
def parse_nmap_xml(path: str | Path) -> dict:
    ...
    for h in root.findall('host'):
        ...
        if addr is None: addr=h.find('address')
        if addr is None: continue
        hn=''
        hostnames=h.find('hostnames')
        if hostnames is not None:
            first=hostnames.find('hostname')
            if first is not None: hn=first.get('name','')
        services=[]
        ports=h.find('ports')
        if ports is not None:
            for p in ports.findall('port'):
                ...
                svc=p.find('service')
                services.append({'port': int(p.get('portid','0')), 'protocol': p.get('protocol','tcp'), 'state':'open', 'name': svc.get('name','') if svc is not None else '', 'product': svc.get('product','') if svc is not None else '', 'version': svc.get('version','') if svc is not None else ''})
