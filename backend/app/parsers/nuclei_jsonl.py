    ...
    for line in path.read_text(errors='ignore').splitlines():
        ...
        out.append(
            NucleiParsedFinding(
                ...
                matcher_name=raw.get('matcher-name') or raw.get('matcher_name'),
                matched_at=raw.get('matched-at') or raw.get('matched_at'),
                host=raw.get('host'),
                ip=raw.get('ip'),
                port=port,
                description=str(info.get('description') or ''),
                tags=_list(info.get('tags') or raw.get('tags')),
                references=_list(info.get('reference') or info.get('references') or raw.get('reference')),
                extracted_results=_list(raw.get('extracted-results') or raw.get('extracted_results')),
                raw_json=raw,
            )
        )
    return out
