...
from app.jobs.netexec_job import run_netexec_job, ALLOWED_TEMPLATES
...
async def approve_action(mission_id:str, action_id:str, payload:ApproveActionPayload, db:Session=Depends(get_db)):
    ...
    if a.command_template_id not in ALLOWED_TEMPLATES: raise HTTPException(403,'Action template not allowed')
