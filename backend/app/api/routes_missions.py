...
from app.capabilities.catalog import creates_manual_card_only, get_capability
...
class MissionCreate(BaseModel):
    name: str
    scenario: str = 'windows_ad_internal'
    mode: str = 'safe'
    scope: str


def serialize_mission(db: Session, m: Mission):
    hosts = db.query(Host).filter_by(mission_id=m.id).all()
    services = db.query(Service).filter_by(mission_id=m.id).all()
    ev_rows = db.query(Evidence.category).filter_by(mission_id=m.id).all()
    ev_cats = {}
    [ev_cats.__setitem__(r[0], ev_cats.get(r[0], 0) + 1) for r in ev_rows]
    by_host = {h.id: [] for h in hosts}
    for s in services:
        by_host.setdefault(s.host_id, []).append(
            {
                'id': s.id,
                'port': s.port,
                'protocol': s.protocol,
    ...
    return {
        ...
        'evidence_summary': {'count': len(ev_rows), 'categories': ev_cats},
        'jobs': [
            {
                'id': j.id,
            ...
            for j in db.query(Job).filter_by(mission_id=m.id).all()
        ],
        'hosts': [
            {
                'id': h.id,
                ...
                'smb_facts': [
                    ...
                    for sf in db.query(SMBFact).filter_by(mission_id=m.id, ip=h.ip).all()
                ],
                'smb_shares': [
                    {
                        'name': sh.name,
                    ...
                    for sh in db.query(SMBShare).filter_by(mission_id=m.id, ip=h.ip).all()
                ],
            }
            for h in hosts
        ],
        'web_targets': [
            ...
            for w in db.query(WebTarget).filter_by(mission_id=m.id).all()
        ],
        'findings': [
            {
                'id': f.id,
            ...
            for f in db.query(Finding).filter_by(mission_id=m.id).all()
        ],
        'next_actions': [
            {
                'id': a.id,
            ...
            for a in db.query(NextAction).filter_by(mission_id=m.id).all()
        ],
    }
...
def _bh_command(mission_id: str) -> dict:
    ...
    return {
        ...
        'risk_level': 3,
        'requires_domain_user_context': True,
        'notes': [
            'Exécuter uniquement dans un environnement autorisé.',
            'SharpHound doit être lancé dans un contexte utilisateur domaine.',
            'Importer le ZIP généré dans OpenAD Zero.',
        ],
    }
...
def _ser_collection(c: BloodHoundCollection):
    return {
        ...
        'ingestion_enabled': c.ingestion_enabled,
        'ingestion_status': c.ingestion_status,
        'ingestion_job_id': c.ingestion_job_id,
        'ingestion_error': c.ingestion_error,
...
async def sharphound_command(mission_id: str, db: Session = Depends(get_db)):
    ...
    await publish(
        MissionEvent(type='bloodhound.command.generated', mission_id=mission_id, payload={'command': data['command']})
    )
    return data
...
def bloodhound_status(mission_id: str, db: Session = Depends(get_db)):
    if not db.get(Mission, mission_id):
        raise HTTPException(404, 'Mission not found')
    return {
        'enabled': get_settings().bloodhound_enabled,
        'status': 'disabled' if not get_settings().bloodhound_enabled else 'enabled',
        'base_url': get_settings().bloodhound_base_url,
...
def get_bh_collection(mission_id: str, collection_id: str, db: Session = Depends(get_db)):
    ...
    if not c or c.mission_id != mission_id:
...
async def upload_bh_zip(mission_id: str, file: UploadFile = File(...), db: Session = Depends(get_db)):
    ...
    c = BloodHoundCollection(
        mission_id=mission_id,
        status='created',
        source='upload',
        filename=file.filename,
        ingestion_enabled=get_settings().bloodhound_enabled,
    )
    db.add(c)
    db.commit()
    db.refresh(c)
    ...
    max_bytes = get_settings().bloodhound_max_upload_mb * 1024 * 1024
    size = 0
    await publish(
        MissionEvent(type='bloodhound.upload.started', mission_id=mission_id, payload={'filename': file.filename})
    )
    with dest.open('wb') as out:
        while True:
            ...
            size += len(chunk)
            if size > max_bytes:
                raise HTTPException(413, 'ZIP upload too large')
            out.write(chunk)
    if size == 0:
        raise HTTPException(400, 'Empty ZIP file refused')
    c.stored_path = str(dest)
    c.size_bytes = size
    c.uploaded_at = datetime.utcnow()
    c.status = 'uploaded'
    c.sha256 = sha256_file(dest)
    (base / 'sha256.txt').write_text(c.sha256 + '\n')
    summary = inspect_sharphound_zip(dest)
    c.zip_summary_json = summary
    c.zip_valid = summary['valid']
    c.validated_at = datetime.utcnow()
    c.status = 'validated' if c.zip_valid else 'invalid'
    (base / 'zip_summary.json').write_text(json.dumps(summary, indent=2))
    await publish(
        MissionEvent(
            type='bloodhound.zip.validated',
            mission_id=mission_id,
            payload={
                'collection_id': c.id,
                'valid': c.zip_valid,
                'json_files_count': summary['json_files_count'],
                'sha256': c.sha256,
            },
        )
    )
    ...
    if c.zip_valid:
        try:
            mark_phase_completed(db, mission_id, 'active_directory_collection', 'BloodHound ZIP validated.')
            create_timeline_event(
                db,
                mission_id,
                TimelineEventCreate(
                    event_type='bloodhound.collection.validated',
                    title='BloodHound collection uploaded and validated',
                    source='bloodhound',
                    severity='success',
                    related_bloodhound_collection_id=c.id,
                ),
            )
        except Exception:
            log.exception('operations bloodhound hook failed')
        await publish(
            MissionEvent(
                type='bloodhound.ingestion.started',
                mission_id=mission_id,
                payload={'collection_id': c.id, 'provider': 'bloodhound-ce'},
            )
        )
        result = await ingest_collection(c)
        event = (
            'bloodhound.ingestion.completed'
            if c.ingestion_status == 'ingested' or c.ingestion_status == 'bloodhound_disabled'
            else 'bloodhound.ingestion.failed'
        )
        await publish(
            MissionEvent(
                type=event,
                mission_id=mission_id,
                payload={'collection_id': c.id, 'status': c.ingestion_status, 'error': c.ingestion_error},
            )
        )
        plan_after_bloodhound_upload(db, mission_id, c.zip_valid)
    (base / 'ingestion_result.json').write_text(json.dumps(result, indent=2))
    (base / 'stats.json').write_text(json.dumps(empty_stats(), indent=2))
    _write_readme(base, c, get_settings().bloodhound_enabled)
    db.add(c)
    db.commit()
    db.refresh(c)
    return _ser_collection(c)
...
async def reingest_bh_collection(mission_id: str, collection_id: str, db: Session = Depends(get_db)):
    ...
    if not c or c.mission_id != mission_id:
        raise HTTPException(404, 'Collection not found')
    if not c.zip_valid:
        raise HTTPException(400, 'Collection ZIP is not valid')
    res = await ingest_collection(c)
    db.commit()
    return {'collection': _ser_collection(c), 'result': res}


@router.post('')
def create_mission(payload: MissionCreate, db: Session = Depends(get_db)):
    try:
        targets = validate_scope(payload.scope, get_settings().allow_public_scans).targets
    ...
    try:
        ...
        log.exception('operations init failed')
...
async def approve_action(mission_id: str, action_id: str, payload: ApproveActionPayload, db: Session = Depends(get_db)):
    if not payload.approved:
        raise HTTPException(400, 'approved must be true')
    if not db.get(Mission, mission_id):
        raise HTTPException(404, 'Mission not found')
    a = db.get(NextAction, action_id)
    if not a or a.mission_id != mission_id:
        raise HTTPException(404, 'Action not found')
    if not a.requires_approval:
        raise HTTPException(400, 'Action does not require approval')
    if a.risk_level > 2:
        raise HTTPException(403, 'Action préparée mais exécution automatique désactivée en V2.')
    if a.command_template_id in NETEXEC_TEMPLATES:
        j = Job(
            mission_id=mission_id,
            type=a.command_template_id,
            tool='netexec',
            status='pending',
            command_preview='nxc smb <nmap-smb-targets> <safe-template>',
        )
        a.status = 'running'
        db.add(j)
        db.commit()
        ...
        if not targets:
            raise HTTPException(400, 'No web targets available')
        j = Job(
            mission_id=mission_id,
            type=a.command_template_id,
            tool='nuclei',
            status='pending',
            command_preview='nuclei -list <web-targets> -jsonl -o <evidence>/nuclei.jsonl <safe-options>',
        )
        a.status = 'running'
        db.add(j)
        db.commit()
        ...
        raise HTTPException(403, 'Action template not allowed')
...
def _validate_manual_payload(payload):
    ...
    if not creates_manual_card_only(cap):
        raise HTTPException(400, 'Capability does not allow manual action cards')
    blob = ' '.join(
        str(getattr(payload, k, '')) for k in ('title', 'description', 'operator_note', 'evidence_reference')
    ).lower()
    banned = [
        'command',
        'cmd.exe',
    ...
    if any(x in blob for x in banned):
        raise HTTPException(
            400,
            'Manual action cards document outcomes only and cannot contain executable commands or sensitive instructions',
        )
    return cap
...
def create_manual_action(mission_id: str, payload: ManualActionCreate, db: Session = Depends(get_db)):
    if not get_settings().openadzero_enable_manual_action_cards:
        raise HTTPException(403, 'Manual action cards disabled')
    if not db.get(Mission, mission_id):
        raise HTTPException(404, 'Mission not found')
    cap = _validate_manual_payload(payload)
    card = ManualActionCard(
        mission_id=mission_id,
...
def list_manual_actions(mission_id: str, db: Session = Depends(get_db)):
    ...
    return [
        _ser_manual(c)
        for c in db.query(ManualActionCard)
        .filter_by(mission_id=mission_id)
        .order_by(ManualActionCard.created_at.desc())
        .all()
    ]
...
def get_manual_action(mission_id: str, manual_action_id: str, db: Session = Depends(get_db)):
    ...
    if not c or c.mission_id != mission_id:
        raise HTTPException(404, 'Manual action card not found')
    return _ser_manual(c)


@router.patch('/{mission_id}/manual-actions/{manual_action_id}')
def update_manual_action(
    mission_id: str, manual_action_id: str, payload: ManualActionUpdate, db: Session = Depends(get_db)
):
    c = db.get(ManualActionCard, manual_action_id)
    if not c or c.mission_id != mission_id:
        raise HTTPException(404, 'Manual action card not found')
    if payload.status and payload.status not in {'draft', 'documented', 'reviewed', 'rejected'}:
        raise HTTPException(400, 'Invalid manual action status')
    if payload.capability_id if hasattr(payload, 'capability_id') else False:
        raise HTTPException(400, 'Capability cannot be changed')
    _validate_manual_payload(
        type(
            'P',
            (),
            {
                ...
                'operator_note': payload.operator_note or c.operator_note,
                'evidence_reference': payload.evidence_reference or c.evidence_reference,
            },
        )()
    )
...
async def start_mission(mission_id: str, db: Session = Depends(get_db)):
    ...
    if not m:
        raise HTTPException(404, 'Mission not found')
    if m.status != 'scope_validated':
        raise HTTPException(400, 'Mission scope is not validated or mission already started')
    m.status = 'running'
    m.started_at = datetime.utcnow()
    try:
        initialize_operations_for_mission(db, m.id)
        mark_phase_completed(db, m.id, 'scope_validation', 'Mission scope validated.')
        update_phase_status(db, m.id, 'network_discovery', 'running', 'Nmap discovery started.')
        create_timeline_event(
            db,
            m.id,
            TimelineEventCreate(
                event_type='nmap.started', title='Nmap discovery started', source='nmap', severity='info'
            ),
        )
    except Exception:
        log.exception('operations start hook failed')
    j = Job(
        mission_id=m.id,
        type='nmap_discovery',
        tool='nmap',
        status='pending',
        command_preview='nmap -Pn -sV --top-ports 1000 -oX <evidence-path> <validated-targets>',
    )
    db.add(j)
    db.commit()
    db.refresh(j)
...
def list_missions(db: Session = Depends(get_db)):
    return [
        ...
        for m in db.query(Mission).order_by(Mission.created_at.desc()).all()
    ]
    ...
    return [
        {
            ...
            'scheme': w.scheme,
            'source': w.source,
            'findings_count': db.query(Finding).filter_by(mission_id=mission_id, ip=w.ip, source='nuclei').count(),
        }
...
def list_findings(mission_id: str, db: Session = Depends(get_db)):
    ...
    return [
        ...
        for f in db.query(Finding).filter_by(mission_id=mission_id).all()
    ]
...
def _explorer(db: Session):
    return BloodHoundExplorer(db)


def _bh_exc(e: Exception):
    if isinstance(e, QueryCatalogError):
        raise HTTPException(400, str(e))
    if isinstance(e, BloodHoundError):
...
async def bloodhound_explorer_status(mission_id: str, db: Session = Depends(get_db)):
    ...
    await publish(
        MissionEvent(
            type='bloodhound.explorer.ready',
            mission_id=mission_id,
            payload={'enabled': data['enabled'], 'reachable': data['reachable'], 'ingested': data['ingested']},
        )
    )
    return data
...
async def bloodhound_search_objects(
    mission_id: str, q: str, types: str | None = None, limit: int = 20, db: Session = Depends(get_db)
):
    if not db.get(Mission, mission_id):
        raise HTTPException(404, 'Mission not found')
    try:
        return await _explorer(db).search_objects(
            mission_id, q, min(limit, 50), [x for x in (types or '').split(',') if x]
        )
...
async def bloodhound_object_detail(mission_id: str, object_id: str, db: Session = Depends(get_db)):
    if not db.get(Mission, mission_id):
        raise HTTPException(404, 'Mission not found')
    try:
        return await _explorer(db).object_detail(mission_id, object_id)
    except Exception as e:
        _bh_exc(e)


@router.get('/{mission_id}/bloodhound/objects/{object_id}/relations')
async def bloodhound_object_relations(
    mission_id: str, object_id: str, direction: str = 'outbound', limit: int = 100, db: Session = Depends(get_db)
):
    if not db.get(Mission, mission_id):
        raise HTTPException(404, 'Mission not found')
    ...
    try:
        return await _explorer(db).relations(mission_id, object_id, direction, min(limit, 200))
    except Exception as e:
        _bh_exc(e)


@router.get('/{mission_id}/bloodhound/objects/{object_id}/permissions')
async def bloodhound_object_permissions(
    mission_id: str, object_id: str, limit: int = 100, db: Session = Depends(get_db)
):
    if not db.get(Mission, mission_id):
        raise HTTPException(404, 'Mission not found')
    try:
        return await _explorer(db).permissions(mission_id, object_id, min(limit, 200))
    except Exception as e:
        _bh_exc(e)
...
async def bloodhound_pathfinding(mission_id: str, payload: PathfindingPayload, db: Session = Depends(get_db)):
    if not db.get(Mission, mission_id):
        raise HTTPException(404, 'Mission not found')
    try:
        res = await _explorer(db).pathfinding(
            mission_id, payload.source_object_id, payload.target, min(payload.max_depth, 12)
        )
        if res:
            try:
                mark_phase_completed(db, mission_id, 'pathfinding', 'BloodHound pathfinding returned results.')
                create_timeline_event(
                    db,
                    mission_id,
                    TimelineEventCreate(
                        event_type='bloodhound.path.detected',
                        title='BloodHound pathfinding detected a path',
                        source='bloodhound',
                        severity='success',
                    ),
                )
            except Exception:
                log.exception('operations pathfinding hook failed')
        return res
    except Exception as e:
        _bh_exc(e)
...
def list_jobs(mission_id: str, db: Session = Depends(get_db)):
    ...
    return [
        ...
        .filter_by(mission_id=mission_id)
        .order_by(Job.queued_at.desc().nullslast(), Job.started_at.desc().nullslast())
        .all()
    ]
...
def get_job(mission_id: str, job_id: str, db: Session = Depends(get_db)):
    j = db.get(Job, job_id)
    if not j or j.mission_id != mission_id:
...
def get_job_logs(mission_id: str, job_id: str, db: Session = Depends(get_db)):
    ...
    return [
        ...
        for log in db.query(JobLog)
        .filter_by(mission_id=mission_id, job_id=job_id)
...
async def cancel_job(mission_id: str, job_id: str, db: Session = Depends(get_db)):
    ...
    if not j or j.mission_id != mission_id:
        raise HTTPException(404, 'Job not found')
    if j.status not in {'pending', 'queued', 'running'}:
    ...
    await publish_mission_event(
        ...
        {'job_id': j.id, 'job_type': j.type, 'tool': j.tool, 'status': j.status},
        source=j.tool,
    )
...
async def retry_job(mission_id: str, job_id: str, db: Session = Depends(get_db)):
    ...
    if not j or j.mission_id != mission_id:
        raise HTTPException(404, 'Job not found')
    if j.status not in {'failed', 'timeout'}:
        raise HTTPException(400, 'Only failed or timeout jobs can be retried')
    if (j.attempts or 0) >= (j.max_attempts or 1):
        raise HTTPException(400, 'Maximum retry attempts reached')
    j.status = 'pending'
    j.completed_at = None
    j.started_at = None
    j.cancel_requested_at = None
    ...
    await publish_mission_event(
        ...
        mission_id,
        'job.retried',
        {'job_id': j.id, 'job_type': j.type, 'tool': j.tool, 'status': j.status, 'rq_job_id': rqid},
        source=j.tool,
    )
    return _serialize_job(j)
