...
def recommendation_catalog():
    ...
    return {
        ...
        'safety_policy': {
            ...
            'v2_current_execution_policy': policy.get('v2_current_execution_policy', {}),
        },
    }


@router.get('/v2/scans/{scan_id}/recommendations', response_model=list[V2Recommendation])
def scan_recommendations(scan_id: str, db: Session = Depends(get_db)):
    scan = _scan_or_404(db, scan_id)
    events = db.query(ScanEvent).filter_by(scan_id=scan_id).order_by(ScanEvent.created_at.asc()).all()
    artifacts = db.query(ScanArtifact).filter_by(scan_id=scan_id).order_by(ScanArtifact.created_at.asc()).all()
