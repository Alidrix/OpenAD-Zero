...
def serialize_event(e: MissionEvent):
    return {'id':e.id,'mission_id':e.mission_id,'event_type':e.event_type,'type':e.event_type,'source':e.source,'payload_json':e.payload_json,'payload':e.payload_json or {},'created_at':e.created_at}
...
def query_events(db, mission_id, after_id=None, limit=200, event_type=None, source=None):
    ...
    if after_id:
        ...
        if marker: q=q.filter(MissionEvent.created_at>marker.created_at)
    if event_type: q=q.filter(MissionEvent.event_type==event_type)
    if source: q=q.filter(MissionEvent.source==source)
    return q.order_by(MissionEvent.created_at.asc()).limit(limit).all()

@router.get('/api/missions/{mission_id}/events')
def list_events(mission_id:str, after_id:str|None=None, limit:int=Query(200, le=1000), event_type:str|None=None, source:str|None=None, db:Session=Depends(get_db)):
...
def recent_events(mission_id:str, limit:int=Query(200, le=1000), event_type:str|None=None, source:str|None=None, db:Session=Depends(get_db)):
...
async def ws_mission(websocket: WebSocket, mission_id: str):
    ...
    try:
        if websocket.query_params.get('replay') == 'true':
            db=SessionLocal()
            try:
                for e in query_events(db, mission_id, websocket.query_params.get('after_id'), int(websocket.query_params.get('limit') or 200)):
