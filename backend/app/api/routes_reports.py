...
from app.reports.schemas import ReportGenerateRequest, ReportPreviewResponse, ReportResponse
...
async def generate(mission_id: str, payload: ReportGenerateRequest | None = None, db: Session = Depends(get_db)):
    ...
    try:
        ...
        try:
            mark_phase_completed(db, mission_id, 'reporting', 'Report generated.')
            create_timeline_event(
                ...
                TimelineEventCreate(
                    ...
                    source='report',
                    severity='success',
                    related_report_id=r.id,
                ),
            )
        except Exception:
            log.exception('operations report hook failed')
        await publish(
            MissionEvent(
                type='report.generated',
                mission_id=mission_id,
                payload={'report_id': r.id, 'markdown_path': r.markdown_path, 'html_path': r.html_path},
            )
        )
        return _ser(r)
...
def preview(
    mission_id: str, format: str = Query('markdown', pattern='^(markdown|html)$'), db: Session = Depends(get_db)
):
    r = get_latest_report(db, mission_id)
    if not r:
        raise HTTPException(404, 'No report generated yet')
    p = get_report_file(r, format)
...
def download(
    ...
    return FileResponse(
        p, filename=f'openadzero_{mission_id}_report.{ext}', media_type='text/markdown' if ext == 'md' else 'text/html'
    )
