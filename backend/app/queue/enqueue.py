...
def _persist_queue_event(db: Session, mission_id: str, job: Job) -> None:
    ...
    try:
        ...
        sid=get_redis_connection().xadd(f'openadzero:mission:{mission_id}:events', {'event_id':event.id,'event_type':event.event_type,'source':event.source,'payload_json':json.dumps(payload),'created_at':event.created_at.isoformat()})
        event.redis_stream_id=sid.decode() if isinstance(sid,bytes) else str(sid); db.commit()
...
def enqueue_job(db: Session, mission_id: str, job_id: str, job_type: str) -> str:
    ...
    if not job:
        ...
        job = Job(id=job_id, mission_id=mission_id, type=job_type, tool=tool, status='pending', command_preview='queued worker job')
        db.add(job); db.commit(); db.refresh(job)
    rq_job = get_scan_queue().enqueue('app.queue.tasks.run_queued_job', job.id, job_timeout=3600, result_ttl=86400, failure_ttl=86400)
    job.rq_job_id = rq_job.id; job.status='queued'; job.queued_at=datetime.utcnow(); job.error_message=None
