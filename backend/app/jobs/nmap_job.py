...
from app.db.models import Host, Job, Mission, Service
...
async def run_nmap_job(mission_id: str, job_id: str):
    ...
    try:
        ...
        db.commit()
        await publish(
            MissionEvent(type='job.status', mission_id=mission_id, payload={'job_id': job_id, 'status': 'running'})
        )
        ...
        job.command_preview = 'nmap ' + ' '.join(args)
        job.stdout_path = str(out)
        job.stderr_path = str(err)
        job.output_path = str(xml)
        db.commit()
        await publish(
            MissionEvent(
                type='job.log',
                mission_id=mission_id,
                payload={'job_id': job_id, 'line': '[nmap] Starting safe discovery scan'},
            )
        )
        ...
        if res.timed_out:
            ...
            for h in parsed['hosts']:
                ...
                await publish(
                    MissionEvent(
                        ...
                        mission_id=mission_id,
                        payload={'ip': host.ip, 'hostname': host.hostname, 'is_domain_controller_candidate': False},
                    )
                )
                for s in h['services']:
                    svc = Service(mission_id=mission_id, host_id=host.id, **s)
                    db.add(svc)
                    await publish(
                        MissionEvent(
                            type='service.discovered', mission_id=mission_id, payload={'host_ip': host.ip, **s}
                        )
                    )
            db.commit()
            ...
            for w in web_targets:
                await publish(
                    MissionEvent(
                        type='web.target.discovered',
                        mission_id=mission_id,
                        payload={'url': w.url, 'ip': w.ip, 'port': w.port, 'scheme': w.scheme, 'source': w.source},
                    )
                )
        ...
        db.commit()
        await publish(
            MissionEvent(type='job.status', mission_id=mission_id, payload={'job_id': job_id, 'status': job.status})
        )
        ...
        db.commit()
        await publish(
            MissionEvent(type='job.log', mission_id=mission_id, payload={'job_id': job_id, 'line': f'[error] {e}'})
        )
    finally:
        db.close()
