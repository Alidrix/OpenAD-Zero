...
def build_nuclei_command(
    targets_file: Path, jsonl_output: Path, templates_dir: Path, rate_limit: int, concurrency: int, timeout: int
) -> list[str]:
    return [
        'nuclei',
        '-list',
...
async def run_nuclei_job(mission_id: str, job_id: str, action_id: str):
    ...
    try:
        ...
        if not targets:
            ...
            db.commit()
            await publish(
                MissionEvent(type='nuclei.log', mission_id=mission_id, payload={'job_id': job_id, 'line': msg})
            )
            return
        ...
        await publish(
            MissionEvent(
                ...
                payload={'job_id': job_id, 'action_id': action_id, 'targets_count': len(targets)},
            )
        )
        templates = Path(os.getenv('NUCLEI_TEMPLATES_DIR', '/app/nuclei-templates-safe'))
        if not templates.exists():
            templates = Path(__file__).resolve().parents[3] / 'nuclei-templates-safe'
        ...
        validate_nuclei_command(cmd, targets_file, jsonl, jd)
        ...
        parsed_path.write_text(json.dumps([p.to_dict() for p in parsed], indent=2))
        created = []
        for p in parsed:
            wt = db.query(WebTarget).filter_by(mission_id=mission_id, url=p.host or '').first() or (
                db.query(WebTarget).filter_by(mission_id=mission_id, ip=p.ip).first() if p.ip else None
            )
            host = db.query(Host).filter_by(mission_id=mission_id, ip=p.ip).first() if p.ip else None
            f = Finding(
                mission_id=mission_id,
                host_id=host.id if host else (wt.host_id if wt else None),
                title=p.template_name,
                severity=p.severity,
                description=p.description or p.template_name,
                source='nuclei',
                confidence='medium',
                template_id=p.template_id,
                template_name=p.template_name,
                matcher_name=p.matcher_name,
                matched_at=p.matched_at,
                host=p.host,
                ip=p.ip,
                port=p.port,
                scheme=(wt.scheme if wt else None),
                tags=p.tags,
                references=p.references,
                raw_json=p.raw_json,
                raw_event_path=str(jsonl),
                evidence_path=str(jd),
            )
            db.add(f)
            created.append(f)
            await publish(
                MissionEvent(
                    type='nuclei.finding.created',
                    mission_id=mission_id,
                    payload={
                        'template_id': p.template_id,
                        'template_name': p.template_name,
        ...
        findings_path.write_text(
            json.dumps([{'title': f.title, 'severity': f.severity, 'source': f.source} for f in created], indent=2)
        )
        ...
        db.commit()
        await publish(
            MissionEvent(type='nuclei.log', mission_id=mission_id, payload={'job_id': job_id, 'line': f'[nuclei] {e}'})
        )
    finally:
        db.close()
