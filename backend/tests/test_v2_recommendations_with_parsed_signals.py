...
def test_parsed_signal_smb_recommends_smb_review(db_session):
    ...
    db_session.add(scan)
    db_session.flush()
    db_session.add(ParsedSignal(scan_id=scan.id, source_type='test', signal='smb_open', value='true', confidence=1.0))
    db_session.commit()
...
def test_recommendations_fallback_events_still_work(db_session):
    ...
    db_session.add(scan)
    db_session.flush()
    ev = ScanEvent(scan_id=scan.id, event_type='service', message='port 445 smb', payload_json={})
    db_session.add(ev)
    db_session.commit()
