...
def test_allowed_job_type_enqueues(monkeypatch, db_session):
    ...
    j = Job(mission_id=m.id, type='nmap_discovery', tool='nmap', status='pending', command_preview='nmap')
    db_session.add(j)
    db_session.commit()
    db_session.refresh(j)
    assert enqueue_job(db_session, m.id, j.id, j.type) == 'rq-123'
    db_session.refresh(j)
    assert j.status == 'queued' and j.rq_job_id == 'rq-123' and j.queued_at is not None


def test_unknown_job_type_refused(db_session):
    m = mission(db_session)
    with pytest.raises(ValueError):
        enqueue_job(db_session, m.id, 'job-x', 'unknown')
