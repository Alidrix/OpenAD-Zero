...
def _scan(db_session, event_type: str, message: str):
    scan = scan_service.create_scan(
        db_session, ScanCreate(name="Signals", scan_type="manual")
    )
    event = ScanEvent(
        scan_id=scan.id, event_type=event_type, message=message, payload_json={}
    )
...
def test_smb_open_recommends_smb_review(db_session):
    scan, events = _scan(
        db_session, "service.detected", "SMB port 445 open on Windows host"
    )
...
def test_scan_without_signal_has_no_recommendations(db_session):
    scan = scan_service.create_scan(
        db_session, ScanCreate(name="No signal", scan_type="manual")
    )
