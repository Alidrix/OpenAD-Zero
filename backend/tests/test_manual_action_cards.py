...
def test_create_list_update_manual_action_card(db_session, monkeypatch):
    ...
    mission(db_session)
    payload=ManualActionCreate(capability_id='manual_lab_validation_notes',title='Manual lab validation note',description='Manual validation performed outside OpenAD Zero in an authorized lab environment.',risk_level=5,operator_note='',evidence_reference='')
    card=create_manual_action('m-manual',payload,db_session)
    assert card['capability_id']=='manual_lab_validation_notes'; assert card['status']=='draft'
...
def test_cannot_create_executable_command_through_manual_endpoint(db_session):
    mission(db_session)
    with pytest.raises(HTTPException):
        create_manual_action('m-manual',ManualActionCreate(capability_id='manual_lab_validation_notes',title='run command',description='powershell command here',risk_level=5),db_session)
