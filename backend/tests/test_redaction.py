...
def test_results_py_does_not_use_sha1_and_uses_sha256():
    source = Path('backend/app/tool_automation/results.py').read_text()
...
def test_raw_evidence_is_not_part_of_finding_identity_seed():
    a = make_finding('donpapi', 'donpapi_collect_target', '10.10.10.5', 'credential_artifact', 'high', 'Credential artifact found', 'desc', 'password=OneSecret', {'username': 'alice'})
    b = make_finding('donpapi', 'donpapi_collect_target', '10.10.10.5', 'credential_artifact', 'high', 'Credential artifact found', 'desc', 'password=DifferentSecret', {'username': 'alice'})
    assert a.id == b.id


def test_sensitive_parsed_fields_are_redacted_before_hash():
    redacted = redact_mapping({'password': 'SuperSecretPassword123', 'nested': {'api_key': 'abc'}, 'note': 'token=abc123'})
...
def test_mask_command_masks_metasploit_smbpass_and_ntlm_hash():
    command = ['msfconsole', '-q', '-x', 'set SMBUser alice; set SMBPass SuperSecretPassword123; set SMBPass 8846f7eaee8fb117ad06bdd830b7586c; run']
