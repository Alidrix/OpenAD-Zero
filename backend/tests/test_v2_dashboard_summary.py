...
from app.db.models import Job, ParsedAsset, ParsedService, ParsedSignal, Scan
...
def test_v2_dashboard_summary_empty_and_shape(client):
    ...
    try:
        ...
        for key in ('scans', 'parsed', 'signals', 'services', 'assets', 'ad_surface'):
            assert key in data
        assert data['parsed'] == {'assets': 0, 'services': 0, 'findings': 0, 'signals': 0, 'diagnostics': 0}
...
def test_v2_dashboard_summary_aggregates_parsed_rows_and_filters_scan_id(client, monkeypatch):
    ...
    try:
        ...
        db.flush()
        asset = ParsedAsset(scan_id=scan_one.id, source_type='test', ip_address='10.0.0.10', os_family='windows', confidence=0.9)
        db.add(asset)
        db.flush()
        db.add_all([
            ParsedService(scan_id=scan_one.id, asset_id=asset.id, source_type='test', ip_address='10.0.0.10', port=445, protocol='tcp', service_name='smb', state='open', confidence=0.9),
            ParsedSignal(scan_id=scan_one.id, asset_id=asset.id, source_type='test', signal='ldap_open', value='true', confidence=0.9),
            ParsedSignal(scan_id=scan_one.id, asset_id=asset.id, source_type='test', signal='kerberos_open', value='true', confidence=0.9),
            ParsedAsset(scan_id=scan_two.id, source_type='test', ip_address='10.0.0.20', confidence=0.9),
        ])
        db.commit()
