...
from app.db.models import BloodHoundCollection, Finding, Mission
...
class FakeClient:
    def __init__(self, result=None, enabled=True, reachable=True):
        self.result = result or {'rows': []}
    ...
    async def run_cypher_query(self, query, include_properties=True):
        self.queries.append(query)
        return self.result


def mission(db):
    m = Mission(id='mx', name='m', scenario='s', mode='safe', raw_scope='10.0.0.1', validated_targets=[])
    db.add(m)
    db.commit()
    return m
...
def test_query_catalog_loads_and_validates_limit():
    c = QueryCatalog()
    assert c.get('search_objects')['read_only'] is True
    ...
    with pytest.raises(QueryCatalogError):
        c.get('raw cypher from frontend')
    with pytest.raises(QueryCatalogError):
        c.render('object_detail', {})
...
def test_mapper_objects_and_edges():
    ...
    g = map_object(
        {'labels': ['Group'], 'properties': {'objectid': 'G-1', 'name': 'DOMAIN ADMINS@LAB', 'highvalue': True}}
    )
    ...
    async def run():
        ...
        e.client.result = {
            'rows': [
                {
                    'nodes': [
                        {'object_id': 'S-1', 'name': 'USER@LAB'},
                        {'object_id': 'G-1', 'name': 'DOMAIN ADMINS@LAB'},
                    ],
                    'edges': [relrow],
                }
            ]
...
def test_explorer_status_disabled_unreachable(db_session):
    async def run():
        ...
        db_session.add(BloodHoundCollection(mission_id='mx', ingestion_status='ingested', ingestion_enabled=True))
        db_session.commit()
