...
def test_publish_mission_event_writes_db(monkeypatch, db_session):
    ...
    m=Mission(name='m',scenario='s',mode='safe',status='running',raw_scope='x',validated_targets=['x']); db_session.add(m); db_session.commit(); db_session.refresh(m)
...
def test_event_listing_filters_and_limit(db_session):
    m=Mission(name='m',scenario='s',mode='safe',status='running',raw_scope='x',validated_targets=['x']); db_session.add(m); db_session.commit(); db_session.refresh(m)
    db_session.add_all([MissionEvent(mission_id=m.id,event_type='a',source='s1',payload_json={}),MissionEvent(mission_id=m.id,event_type='b',source='s2',payload_json={})]); db_session.commit()
