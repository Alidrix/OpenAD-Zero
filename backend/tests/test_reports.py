...
from app.db.models import BloodHoundCollection, Evidence, Finding, Host, Mission, Service
...
def seed(db):
    ...
    h = Host(
        ...
        status='up',
        os_guess='Windows',
        is_domain_controller_candidate=True,
    )
    db.add(h)
    db.commit()
    db.refresh(h)
    ...
    db.add(
        Finding(
            mission_id=m.id,
            host_id=h.id,
            title='Nuclei critical exposure',
            severity='critical',
            description='web issue',
            source='nuclei',
            confidence='high',
            template_id='exposure/test',
            matched_at='http://10.0.0.10',
            ip='10.0.0.10',
            port=80,
        )
    )
    db.add(
        Finding(
            mission_id=m.id,
            host_id=h.id,
            title='BloodHound path to privileged group',
            severity='high',
            description='path',
            source='bloodhound',
            confidence='medium',
    ...
    db.add(
        BloodHoundCollection(
            ...
            status='validated',
            filename='bh.zip',
            stored_path='/tmp/bh.zip',
            sha256='b' * 64,
            zip_valid=True,
            ingestion_status='bloodhound_disabled',
        )
    )
    db.commit()
    return m
...
def test_report_api_endpoints(monkeypatch, tmp_path):
    ...
    try:
        ...
        res = client.post(f'/api/missions/{mission_id}/report/generate', json={'include_sections': None})
        assert res.status_code == 200, res.text
        assert client.get(f'/api/missions/{mission_id}/report').json()['report']['id'] == res.json()['id']
        pm = client.get(f'/api/missions/{mission_id}/report/preview?format=markdown')
        assert pm.status_code == 200 and 'Report Mission' in pm.json()['content']
        ph = client.get(f'/api/missions/{mission_id}/report/preview?format=html')
        assert ph.status_code == 200 and '<html' in ph.json()['content'].lower()
        dm = client.get(f'/api/missions/{mission_id}/report/download?format=markdown')
        assert dm.status_code == 200 and b'Report Mission' in dm.content
        dh = client.get(f'/api/missions/{mission_id}/report/download?format=html')
        assert dh.status_code == 200 and b'<html' in dh.content.lower()
    finally:
...
def test_report_includes_operations_sections(db_session):
    ...
    r = generate_report(db_session, m.id)
    with open(r.markdown_path, encoding='utf-8') as report_file:
        md = report_file.read()
